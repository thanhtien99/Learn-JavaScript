...
// so sánh thông tin dữ liệu // menu[i] là mảng được lưu vào trong local storege
  for (i in menu) {
    if (menu[i].category == category) {
      for (j = 0; j < menu[i].list.length; j++) {
        if (menu[i].list[j].id == id) {
...
// lưu thông tin đã chọn vào Local Storage
  order.push(item);
